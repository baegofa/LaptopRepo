...
//단어를 입력받아 저장하는 메소드
	public void updatePlayerWord(int count,String input) {
		playerWordList[count%3][1] = input;
	}
//단어를 비교하는 메소드
	public boolean compareWord(int count) {
		...
		if(count<1) return true;
			...
			if(count%3 == 0) {
				...
				lastWord = playerWordList[count%3-1][wordSection];
				nowWord = playerWordList[count%3][wordSection];
				wordIndex = lastWord.length();
				lastchar = lastWord.charAt(wordIndex-1);
				nowchar = nowWord.charAt(0);
			}
...
//패자를 가리는 메소드
	public void decideLoser(int count) {
		loser = playerWordList[count%3][0];
...
//실행하는 메소드
	public void run() {
		...
		while(compareWord(count-1)) {
			System.out.println(playerWordList[count%3][nameSection]+"님 단어를 입력하세요:");
